...
def main():
    ...
    if args.dataset in ['COLLAB', 'IMDB-BINARY']: #unlabeled
        trs.append(torch_geometric.transforms.Constant()) 
    if args.dataset == 'COLLAB': trs.append(transforms.ToBinaryLabels())
    ...
    if not args.resample: dataset = [x for x in dataset]
    train_dataset = dataset[len(dataset) // 10:]
    test_dataset = dataset[:len(dataset) // 10]

    
    model = models.Net(train_dataset[0].x.shape[1], args.hidden, out_channels=2, layers=args.layers, use_max_aggr=False).to(args.device)
