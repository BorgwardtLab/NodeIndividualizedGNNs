...
def main():
    ...
    for n in [32]: 
        ...
        for ntrain in [1]: 
            ...
            args.logger = SummaryWriter(log_dir=f'runs/cycles_pin/indiv{args.ind_type}/acc_layers{args.layers}_dim{args.ind_dim}_hidden{args.hidden}_lr{args.lr}_size{n}_train{ntrain}_test{ntest}')

            for ii in range(5):

                print('>', n, ntrain, ii)

                filename = f'cycles_pin_size_{n}_N10'
                nmb_classes = 2
                file = open('../data/cycles/'+filename+'.pickle', 'rb')
