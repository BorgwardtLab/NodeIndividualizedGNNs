...
def main():
    ...
    for n in [16, 32, 64]: 
        ...
        for ntrain in [1,10,100,1000]: 

            args.logger = SummaryWriter(log_dir=f'runs/3regular/indiv{args.ind_type}/layers{args.layers}_dim{args.ind_dim}_hidden{args.hidden}_lr{args.lr}_size{n}_train{ntrain}_test{ntest}')

            for ii in range(5):

                print('>', n, ntrain, ii)

                filename = f'3regular_size_{n}_N10'
                nmb_classes = 2
                file = open('../data/3regular/'+filename+'.pickle', 'rb')
                ...
                model = models.Net(train[0].x.shape[1], args.hidden, out_channels=nmb_classes, layers=args.layers).to(args.device)
